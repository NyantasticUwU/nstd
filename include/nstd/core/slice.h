...
/// # Note
///
/// The returned element pointer can quickly become a dangling pointer if the slice's memory
...
/// # Note
///
/// This function follows the same behaviour rules as `nstd_core_slice_get`.
///
/// # Parameters
///
/// - `const NSTDSlice *const slice` - The slice.
...
/// # Returns
///
/// `NSTDBool is_same` - True if the two slices carry the same data.
...
/// # Returns
///
/// `NSTDBool is_in` - True if the slice contains `element`.
NSTDAPI NSTDBool nstd_core_slice_contains(const NSTDSlice *const slice, const NSTDAnyConst element);
...
/// # Returns
///
/// `NSTDUSize count` - The number of `element`s in `slice`.
NSTDAPI NSTDUSize nstd_core_slice_count(const NSTDSlice *const slice, const NSTDAnyConst element);
...
/// # Returns
///
/// `NSTDUSize index` - The index of the element, -1/usize::MAX if not found.
...
/// # Returns
///
/// `NSTDBool starts_with` - True if `slice` starts with `pattern`.
...
/// # Returns
///
/// `NSTDBool ends_with` - True if `slice` ends with `pattern`.
...
NSTDAPI void nstd_core_slice_fill(NSTDSlice *const slice, const NSTDAnyConst element);

/// Fills a specific range of a slice with `element`.
///
/// # Note
///
/// This function does NOT check that `range` is valid for operating on `slice`.
///
/// # Parameters
///
/// - `NSTDSlice *const slice` - The slice.
