...
NSTDAPI NSTDByte *nstd_std_net_tcp_stream_read(NSTDTCPStream stream, NSTDUSize *size);
...
NSTDAPI NSTDUDPSocket nstd_std_net_udp_socket_bind(const char *const addr);
...
/// Returns: `int errc` - Nonzero on error.
NSTDAPI int nstd_std_net_udp_socket_connect(NSTDUDPSocket socket, const char *const addr);
...
NSTDAPI NSTDByte *nstd_std_net_udp_socket_receive(
...
/// Returns: `NSTDByte *bytes` - The bytes received.
NSTDAPI NSTDByte *nstd_std_net_udp_socket_receive_from(
    NSTDUDPSocket socket,
    const NSTDUSize num,
    NSTDUSize *size,
    char **ip);
...
/// Returns: `int errc` - Nonzero on error.
NSTDAPI int nstd_std_net_udp_socket_send(
    NSTDUDPSocket socket,
    const NSTDByte *const bytes,
    const NSTDUSize num,
    NSTDUSize *size);
...
///     `NSTDUSize *size` - Returns as number of bytes actually sent.
/// Returns: `int errc` - Nonzero on error.
NSTDAPI int nstd_std_net_udp_socket_send_to(
    NSTDUDPSocket socket,
...
NSTDAPI void nstd_std_net_udp_socket_close(NSTDUDPSocket *socket);

/// Deallocates memory where an IP address string is allocated.
/// Parameters:
///     `char **ip` - The IP address.
NSTDAPI void nstd_std_net_free_ip(char **ip);
...
NSTDAPI void nstd_std_net_free_bytes(NSTDByte **bytes, const NSTDUSize size);
